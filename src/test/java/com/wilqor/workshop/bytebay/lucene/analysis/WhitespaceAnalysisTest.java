...
public class WhitespaceAnalysisTest extends FSDirectoryReadingTest {
    ...
    public void shouldRetrieveReviewsForArticleNameTermsWithMatchingCase() throws Exception {
        Query query = new BooleanQuery.Builder()
                .add(new TermQuery(new Term(WhitespaceAnalysisExample.CommentedReviewIndexer.ARTICLE_NAME_FIELD, "Lucene")), BooleanClause.Occur.MUST)
                .add(new TermQuery(new Term(WhitespaceAnalysisExample.CommentedReviewIndexer.ARTICLE_NAME_FIELD, "101")), BooleanClause.Occur.MUST)
                .build();
        TopDocs topDocs = searcher.search(query, QUERY_MATCHES_LIMIT);
    ...
    public void shouldRetrieveReviewsForCommentTerm() throws Exception {
        Query query = new TermQuery(new Term(WhitespaceAnalysisExample.CommentedReviewIndexer.COMMENT_FIELD, "polecam"));
        TopDocs topDocs = searcher.search(query, QUERY_MATCHES_LIMIT);
